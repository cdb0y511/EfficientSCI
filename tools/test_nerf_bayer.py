    ...
    for data_iter,data in enumerate(data_loader):
        ...
        for ii in range(batch_size):
            ...
            for jj in range(cr_list[data_iter]):
                ...
                tmp = lpips_model(torch.tensor(per_frame_gt).unsqueeze(0).float(), torch.tensor(per_frame_out).unsqueeze(0).float()).detach().cpu().numpy()
                lpips_value += tmp[0][0][0][0]

        meas_num = len(batch_output)
        psnr = psnr / (meas_num* cr_list[data_iter])
        ssim = ssim / (meas_num* cr_list[data_iter])
        lpips_value = lpips_value / (meas_num* cr_list[data_iter])
